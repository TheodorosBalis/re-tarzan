...
// Address: 0x00485460
// Label: TrashingTheCamp::InitProjectileDropBird
// Label origin: project
// Original labels and source files are tracked in manifest.csv.

void TrashingTheCamp::InitProjectileDropBird(int entry)

{
  ...
  *(uint *)(entry + 0x28) = *(uint *)(entry + 0x28) | 4;
  if ((*(byte *)(entry + 0x43) & 0x80) == 0) {
    TrashingTheCamp::InitBirdCommon(entry);
    value = CEntities::FindEntityAttachmentByFlags(entry,0x10000000);
    if (value != 0) {
      *(code **)(value + 0xc) = TrashingTheCamp::BirdProjectileDropCallback;
