...
// Address: 0x004051D0
// Label: RockinTheBoat::InitProjectileDropBird
// Label origin: project
// Original labels and source files are tracked in manifest.csv.

void RockinTheBoat::InitProjectileDropBird(int entry)

{
  ...
  *(uint *)(entry + 0x28) = *(uint *)(entry + 0x28) | 4;
  if ((*(byte *)(entry + 0x43) & 0x80) == 0) {
    RockinTheBoat::InitBirdCommon(entry);
    value = CEntities::FindEntityAttachmentByFlags(entry,0x10000000);
    if (value != 0) {
      *(code **)(value + 0xc) = RockinTheBoat::BirdProjectileDropCallback;
