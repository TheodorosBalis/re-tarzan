...
// Address: 0x00401B90
// Label: TheBaboonChase::InitThrowableCarryingBird
// Label origin: project
// Original labels and source files are tracked in manifest.csv.

void TheBaboonChase::InitThrowableCarryingBird(int entry)

{
  ...
  *(uint *)(entry + 0x28) = *(uint *)(entry + 0x28) | 4;
  if ((*(byte *)(entry + 0x43) & 0x80) == 0) {
    TheBaboonChase::InitBirdCommon(entry);
    value = CEntities::FindEntityAttachmentByFlags(entry,0x10000000);
    if (value != 0) {
      *(code **)(value + 0xc) = TheBaboonChase::BirdProjectileDropCallback;
