...
// Address: 0x00427E40
// Label: ElephantHairDare::UpdateTokenOnWater
// Label origin: project
// Original labels and source files are tracked in manifest.csv.

/* WARNING: Globals starting with '_' overlap smaller symbols at the same address */

void ElephantHairDare::UpdateTokenOnWater(int entry)

{
  ...
  if (value != 0) {
    ...
    if (value2 != 0) {
      ...
      value3 = *(int *)(g_PlayerEntitySceneObjectData + 8) - value2;
      if ((int)value3 < 0) {
        value3 = ~value3 + 1;
      }
      if ((int)value3 < 0x800) {
        ElephantHairDare::SpawnTokenOnWaterCollectionEffects
                  (value2,*(undefined4 *)(value + 0x24),
    ...
    if (*(short *)(entry + 0x4a) == 0) {
      resultFlag = CEntities::CheckSceneObjectSphereOverlap3D
                        (entity->sceneObjects,localState,g_PlayerEntitySceneObject,
                         localState2);
      if (resultFlag != '\0') {
        _g_LastCollectedWaterTokenSceneObjectId = (uint)*(ushort *)(entity->sceneObjects + 0xc);
