...
// Address: 0x00438470
// Label: LoadNecessaryEncodedResourceId
// Label origin: project
// Original labels and source files are tracked in manifest.csv.

void LoadNecessaryEncodedResourceId(uint arg1)
  ...
  if ((arg1 & 0x1000) == 0) {
    if ((arg1 & 0x2000) == 0) {
      ...
      if ((value != 0) && (value < 0x91)) {
        LoadNecessaryGSXFontsAndModels(&g_ResourceSetDescriptorTable + (arg1 & 0xfff) * 0x68);
      }
    }
    else {
      value = (ushort)(arg1 & 0xfff);
      if ((value != 0) && (value < 0x204b)) {
        LoadNecessaryWorldSceneAuxResourceNoOp
    ...
    if ((value != 0) && (value < 0x1032)) {
      LoadNecessaryEncodedSceneModelResource
