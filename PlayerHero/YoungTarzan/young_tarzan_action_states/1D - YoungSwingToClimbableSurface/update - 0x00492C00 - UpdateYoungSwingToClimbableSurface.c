...
LAB_00492cf7:
    ...
    if ((g_PlayerInputHeldMask & g_CameraRelativeTurnInputMask) != 0) {
      if (DAT_0051ce40 == '\0') {
        CPlayerHero_PlayAnimation(0x5d);
        CPlayerHero_UpdateMovementAndCollision(1);
        return;
      }
      CPlayerHero_PlayAnimation(0x5e);
      CPlayerHero_UpdateMovementAndCollision(1);
      return;
    }
  ...
  if (DAT_0051ce40 == '\0') {
    CPlayerHero_PlayAnimation(0x56);
    CPlayerHero_UpdateMovementAndCollision(1);
    return;
  }
  CPlayerHero_PlayAnimation(0x5a);
LAB_00492d2b:
  CPlayerHero_UpdateMovementAndCollision(1);
  return;
}
